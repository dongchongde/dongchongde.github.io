...
`%usr` 达到 25-30%，`%iowait` 为 0，说明**瓶颈在应用层，不是磁盘 I/O**。

这一步的作用不是确认 CPU 高，而是**给后续排查指路**：

- `%usr` 高 → 去查应用层代码，用火焰图
- `%sys` 高 → 去查系统调用或网络
- `%iowait` 高 → 瓶颈其实在磁盘，不需要看火焰图

**结论（Utilization）**：压力来自用户态，是应用层代码在消耗 CPU，下一步用火焰图定位。
...
举个反例：如果 `%usr` 60%，但 r=1，说明请求没有积压，用户感受不到慢，这时候去优化代码可能是无效工作。但如果 r 超过核数，请求已经在排队，用户响应时间一定变长了，必须立刻处理。
...
现在 r=3-4，超过了核数 2，属于第四种情况——请求在排队，服务已经受影响，必须立刻找根因。

**结论（Saturation）**：CPU 已饱和，用户响应时间受影响，必须立刻处理。

---

## 第四步：检查内核错误（dmesg）

已经知道了利用率和饱和度，为什么还要看这一步？

因为有些问题利用率和饱和度完全看不出来，只有 dmesg 能发现。比如 CPU 硬件故障（MCE 错误）、OOM Killer 杀进程——这些情况下利用率和饱和度可能完全正常，但服务就是不对劲。dmesg 是一个**兜底检查**，成本只有10秒，关键时刻能避免你在错误方向上浪费大量时间。
...
**结论（Errors）**：无硬件故障，perf 采样率被压低是 CPU 高负载的副作用，下一步直接用火焰图定位根因。
